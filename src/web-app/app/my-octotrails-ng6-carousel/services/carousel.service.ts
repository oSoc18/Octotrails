...
export class CarouselService {
  ...
  public init(imageSources: string[], config: ICarouselConfig) {
    ...
    config.autoplayDelay =
      config.autoplayDelay < 1000 ? 1000 : config.autoplayDelay;
  ...
  private loadImages(imageSources: string[]): void {
    const createImageElement = (image: string): void => {
      ...
      imgElement.onload = this.onImageElementLoad.bind(
        this,
        imageSources,
        image
      );

      imgElement.onerror = this.onImageElementLoadError.bind(
        this,
        imageSources,
        image
      );
  ...
  private onImageElementLoadError(imageSources: string[], image: string): any {
    // console.log(imageSources.indexOf(image));
