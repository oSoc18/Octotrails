...
describe('CarouselService', () => {
  ...
  it('should return copy of carousel config', inject(
    [CarouselService],
  ...
  describe('imageLoad', () => {
    it('should load images via document.createElement', inject(
      [CarouselService],
      (service: CarouselService) => {
        service.init(imageSources, config);
  ...
  describe('defines image success load handler and', () => {
    it('should increase counter of loaded images', inject(
      [CarouselService],
      (service: CarouselService) => {
        service.init(imageSources, config);
    ...
    it('should emit image load event', inject(
      [CarouselService],
      (service: CarouselService) => {
        service.init(imageSources, config);
  ...
  describe('defines image error handler and', () => {
    it('should delete images that not loaded', inject(
      [CarouselService],
      (service: CarouselService) => {
        service.init(imageSources, config);

        console.log(imageSources, imageSources.indexOf('2'));
  ...
  describe('provides init() API', () => {
    it('should set autoplayDelay to 1000 if autoplayDelay < 1000', inject(
      [CarouselService],
      (service: CarouselService) => {
        service.init(imageSources, config);
